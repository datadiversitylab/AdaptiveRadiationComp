...
finch_SAR <- readRDS("Galapagos/Data/finch_SpAR_e0.rds")
scalesia_SAR <- readRDS("Galapagos/Data/scalesia_SpAR_e0.rds")
tetra_SAR <- readRDS("Hawaiian/Data/spider_SpAR_e0.rds")
silversword_SAR <- readRDS("Hawaiian/Data/silver_SpAR_e0.rds")
...
areas <- list(
  ...
  Scalesia          = scalesia_SAR$linObj$model$x,
  Tetragnatha       = tetra_SAR$linObj$model$x,
  Silverswords      = silversword_SAR$linObj$model$x
)
...
write.csv(results, "power/power_spar_lineages.csv", row.names = FALSE)
